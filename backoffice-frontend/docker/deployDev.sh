...
ssh $USERATHOST <<EOF
set -xe
echo "When you see a message like 'AH00489: Apache/2.4.46 (Unix) configured' you may CTL-C to exit."
docker stop $UCNAME
docker rm $UCNAME
docker rmi pharmaledger/$UCNAME
( xz -d < images/$IMG_NAME | docker load )
docker run --detach --hostname $UCNAME --publish 8000:80 --name $UCNAME --restart always pharmaledger/$UCNAME
